...
namespace cm44.DailyH.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly DailyHDbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = DailyHConsts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(DailyHDbContext context)
        {
            _context = context;
        }
